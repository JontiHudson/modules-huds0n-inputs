...
export const DateTimeWeb = React.forwardRef<Types.RefType, Types.Props>(
  (props, ref) => {
    const {
      customError,
      invalidDateError = props.mode === 'time'
        ? 'Invalid time'
        : 'Invalid date',
      mode = 'date',
    ...
    const inputRef = useRef<TextInput.RefType>(null);
    ...
    const { error, onValueChange } = handleValueValidation({
      ...
      validation: useMemo(() => {
        const combinedValidation = validation ? [...toArray(validation)] : [];
      ...
      }, [validation, maximumDate, minimumDate, localError]),
    });
...
function handleFocusBlur(
  { onBlur, onFocus, value }: Types.Props,
